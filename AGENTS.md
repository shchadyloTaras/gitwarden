...
`0→…→20` (MVP) → `21→27` (GitHub OAuth) → `28→39` (AI Connections) → `52→55a` (AI Chat) → `56→59` (Client Branch Access) → `60→62` (GenUI Blocks) → `46→51` (Landing; live at gitwarden.vercel.app) | partial: `63→66` (Guard Quick-Fix; 63–64 done, 65–66 open, guard-quick-fix-plan.md) | partial: `40→45` (Distribution; 43–44 open, gated on signing certs) | DX track: `DX-0→DX-6` (complete, agentic-dx-plan.md)
