...
// asserts the sidebar footer button reflects the result. This proves the core requirement: the
// Update button is shown only when a newer release actually exists.
...
test.describe('Update notifier', () => {
  test('shows the sidebar Update button only after a check finds a newer release', async () => {
    ...
    try {
      ...
      // The sidebar footer button appears, expanded (icon + label), because an update is available.
