...
- Notes / follow-ups: **Safe stop point reached** — the full connect / auto-fill / linked-account experience ships here; Phase 27 (HTTPS push + Safety Engine codes) is the optional remaining epic. The re-auth prompt is wired in the modal (`tokenInvalid` → re-auth copy) but the *detection* of an invalid stored token only becomes active in Phase 27 (the 401 → `GITHUB_TOKEN_INVALID` push-time check). A human should still run `npm run dev` once and authorize a real GitHub account end-to-end (Appendix D) — CI only ever drives the fake.
